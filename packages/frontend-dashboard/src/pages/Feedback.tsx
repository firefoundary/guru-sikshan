...
import { api, type Feedback } from '@/services/api';
...
export function FeedbackPage({ onLogout }: FeedbackProps) {
  ...
  const [selectedIssue, setSelectedIssue] = useState<Feedback | null>(null);

  const fetchIssues = async () => {
    setLoading(true);
    try {
      const result = await api.getFeedback();
      setIssues(result.feedbacks);
    } catch (error) {
      toast({
        title: 'Error',
  ...
  const handleView = (issue: Feedback) => {
  ...
  const inReviewCount = issues.filter(i => i.status === 'in_review' || i.status === 'reviewed').length;
  const resolvedCount = issues.filter(i => i.status === 'resolved' || i.status === 'training_assigned').length;

  return (
    <DashboardLayout 
      title="Feedback" 
      subtitle="Teacher feedback and issue tracking"
      onLogout={onLogout}
    >
      {/* Stats Summary */}
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between mb-6">
        <div>
          <p className="text-muted-foreground">
            Showing <span className="font-semibold text-foreground">{issues.length}</span> feedback items
          </p>
        </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3 pt-4 border-t border-border">
                ...
                <select
                  ...
                  onChange={(e) => {
                    handleStatusChange(selectedIssue.id, e.target.value as Feedback['status']);
                    setSelectedIssue({ ...selectedIssue, status: e.target.value as Feedback['status'] });
                  }}
